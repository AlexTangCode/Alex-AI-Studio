...
const WeightModal: React.FC<WeightModalProps> = ({ henName, onSave, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-xs rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
        <h3 className="text-xl font-black text-slate-800 mb-2 text-center">记录鸡蛋 🥚</h3>
        <p className="text-slate-500 text-sm mb-6 text-center">为 <span className="text-amber-600 font-bold">{henName}</span> 记录产量</p>
        
        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">选择日期</label>
            <input 
              type="date" 
              value={date}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">鸡蛋重量 (可选/克)</label>
            <input 
              type="number" 
              value={weight}
        ...
        <div className="flex flex-col gap-3">
          <button 
            onClick={() => onSave(date, weight ? parseFloat(weight) : undefined)}
